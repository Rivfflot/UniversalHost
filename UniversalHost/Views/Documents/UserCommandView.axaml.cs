...
public partial class UserCommandView : UserControl
{
    private DataGridRow? _currentHoveredRow;
    private static readonly DataFormat<UserCommand> RowFormat =
    DataFormat<UserCommand>.CreateInProcessFormat<UserCommand>("UserCommandRow");
    ...
    private async void Grid_PointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        else
        {
            ...
            if (headerText != "指令" && headerText != "名称")
            {
                return;
            }
            ...
            var row = hit.FindAncestorOfType<DataGridRow>();
            if (row?.DataContext is not UserCommand item)
                return;
    ...
    private void Grid_Drop(object? sender, DragEventArgs e)
    {
        ...
        if (e.DataTransfer.TryGetValue(RowFormat) is not UserCommand dragged) return;

        if (DataContext is not UserCommandViewModel vm) return;
        ...
        int oldIndex = keys.IndexOf(dragged.Id);
        int newIndex = keys.IndexOf(target.Id);
        ...
        vm.SelectedUserCommand = dragged;
    }


    private UserCommand? GetRowUnderPointer(DragEventArgs e, DataGrid? grid)
    {
        if (grid == null)
            return null;
        ...
        var row = hit.FindAncestorOfType<DataGridRow>();
        return row?.DataContext as UserCommand;
    }
}