...
2026-07-29
- 修复Tool窗口关闭后保存再恢复时null引用问题。

2026-08-02
- 创建Tool窗口抽象为方法放进`FockFactory`类。

2026-09-01
- 修改IAP协议，将每帧字节数由上位机决定改为由下位机决定并在握手阶段发送给上位机。原先的IAP字段每帧字节数删除。
- 优化`IapProtocol`类，把传入参数改为路径和设备ID。

2026-09-18
- 更新Nuget包至最新版。
- 修改文件夹名称和命名空间：Views修改为Windows以更加契合实际。同步修改相关文件中的命名空间和引用。

2026-09-21
- 调整逐帧写入设备的 IAP 流程：发送 IAP 信息后等待设备擦除完成及信息 ACK，再开始发送数据帧；擦除等待期间不重复发送信息包。
- 整体写入设备继续在数据发送完成后执行擦除和写入，信息阶段保留原有的快速 ACK 和重试逻辑。
- 新增 IAP 信息阶段等待超时配置及对应设置界面，默认超时时间为 10 秒。

2026-09-25
- 修复工程切换后设置窗口和变量选择窗口仍显示旧工程数据的问题，切换时重新关联集合并清理旧订阅。
- 修复首次打开工程时位监控、曲线监控的已选变量不显示的问题：布局读取与运行时关联分离，在新工程运行时建立后恢复变量、顺序和显示样式，兼容已有工程文件。

2026-09-26
- 连接设备成功后取消自动上传标定变量，改为清空标定值显示和历史记录，避免显示旧值迷惑用户。
